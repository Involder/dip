...
class SubscriberModelForm(forms.ModelForm):
    ...
    speciality = forms.ModelChoiceField ( label='Специальнсоть', queryset=Speciality.objects.all(), widget=forms.Select(attrs={'class': 'form-control'}))
...
class SubscriberHiddenEmailForm(forms.ModelForm):
    ...
    speciality = forms.ModelChoiceField(label='Специальнсоть', queryset=Speciality.objects.all(),
                                        widget=forms.Select(attrs={'class': 'form-control'}))
    password = forms.CharField(label='Пароль', widget=forms.PasswordInput(attrs={'class': 'form-control'}))
...
class ContactForm(forms.Form):
    ...
    speciality = forms.CharField( label='Специальнсоть',required=True, widget=forms.TextInput(attrs={"class": 'form-control'}))